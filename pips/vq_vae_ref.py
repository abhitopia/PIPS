...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_dim, n_head, dropout, rope=None, resample=False):
        super().__init__()
        self.rope = rope
        self.n_dim = n_dim
        self.n_head = n_head
    ...
    def resample_positions(self, positions: Tensor, target_length: int) -> Tensor:
        """
    ...
    def forward(self,
        ...
        if self.rope is not None and positions is not None:
            if self.resample:
                ...
                key_positions = self.resample_positions(positions, kT)  # shape: [B, 1, kT]
                query_positions = self.resample_positions(positions, qT) # shape: [B, 1, qT]
            else:
...
class TransformerBlock(nn.Module):
    def __init__(self, d_model, n_head, rope=None, dim_feedforward=None, dropout=0.0, return_entropy: bool = False, resample=False):
        super().__init__()
        if return_entropy:
            self.mha = MultiHeadAttentionWithEntropy(n_dim=d_model, n_head=n_head, dropout=dropout, rope=rope)
        else:
            self.mha = MultiHeadAttention(n_dim=d_model, n_head=n_head, dropout=dropout, rope=rope, resample=resample)
...
class LatentTransformer(nn.Module):
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            TransformerBlock(d_model=d_model, n_head=n_head, dim_feedforward=dim_feedforward, rope=rope, return_entropy=return_attn_entropy, resample=True)
            for _ in range(n_layer)
        ])
...
class VectorQuantization(Function):
    ...
    def forward(ctx, inputs, codebook):
        ...
        # noise = torch.rand_like(l2_dis) * 0.0  # very small noise
        # l2_dis = l2_dis + noise
